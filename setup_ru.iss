...
[Types]
Name: "compact"; Description: "Стандартная установка"
Name: "custom"; Description: "Пользовательская установка"; Flags: iscustom

[Components]
Name: "MultiverseCore"; Description: "Основной мод"; Types: compact custom; Flags: fixed

Name: "MultiverseCore\TRC"; Description: "Коллекция Ренегата"; Types: custom
Name: "MultiverseCore\SlockFlag"; Description: "Флагман Слокнога"; Types: custom
Name: "MultiverseCore\VanillaUI"; Description: "Возврат ванильного интерфейса"; Types: custom
Name: "MultiverseCore\Holiday"; Description: "Праздничный выпуск"; Types: custom

Name: "MultiverseDiff"; Description: "Моды на сложность"; Types: custom
Name: "MultiverseDiff\Starvation"; Description: "Голод по лому"; Types: custom

Name: "MultiverseThr"; Description: "Сторонние аддоны"; Types: custom
Name: "MultiverseThr\GenGibs"; Description: "Сгенерированные обломки"; Types: custom
...
[Run]
Filename: {app}\downgrade.bat; Description: Run Application; StatusMsg: "Даунгрейд до FTL 1.6.9..."; Flags: runhidden
Filename: "{app}\modman.exe"; Parameters: {code:GetModmanParameters}; WorkingDir: "{app}"; StatusMsg: "Установка выбранных модов..."; Flags: runhidden
Filename: "{app}\FTLGame.exe"; Description: "{cm:LaunchProgram,Multiverse}"; Flags: nowait postinstall skipifsilent runascurrentuser

[UninstallRun]
Filename: "{app}\modman.exe"; Parameters: "--patch ~none~"; StatusMsg: "Удаление Multiverse..."; Flags: runhidden
...
begin
  ...
  if not IsJREInstalled then begin
    if MsgBox('Для установки мода требуется Java версии 1.6 или выше. Перейти на страницу загрузки?', mbConfirmation, MB_YESNO) = IDYES then begin
      ShellExec('open', 'https://www.java.com/en/download/', '', '', SW_SHOWNORMAL, ewNoWait, ErrorCode);
    end;
    while true do begin
      MBResult := MsgBox('После установки Java, нажмите "Повторить", чтобы продолжить установку мода.', mbConfirmation, MB_ABORTRETRYIGNORE or MB_DEFBUTTON2);
      if MBResult = IDAbort then begin
        Result := false;
        MsgBox('Установка была прервана пользователем.', mbConfirmation, MB_OK);
        exit;
      end;
      if (MBResult = IDIgnore) and (MsgBox('Установка модов не будет работать без Java. Вы уверены, что всё равно хотите продолжить?', mbError, MB_YESNO or MB_DEFBUTTON2) = idYes) then break;
...
begin
  ...
  if (CurPageID = wpSelectDir) and not TryPath(WizardDirValue) then begin
    Result := MsgBox('FTLGame.exe не найден в' + WizardDirValue + 'Для корректной установки Multiverse нужно выбрать папку установки игры. Вы всё равно хотите продолжить?',
...
begin
  ...
  '[Обязательно] Включает в себя основные файлы данных для Multiverse.;'+ //CORE
  'Добавляет много новых видов оружия и играбельные корабли, использующие их, с большим количеством уникальных механик и меньшим балансом.;'+ //TRC
  'Добавляет мощные разблокируемый тип Б корабля Флагман Блаженства. Изначально создавался в качестве приза для Рождественского конкурса Дискорда 2021.;'+ //SLOCKNOG'S FLAGSHIP
  'Делает интерфейс Multiverse более похожим на интерфейс игры без модов. Мы советуем по крайней мере попробовать сначала стандартную тему MV.;'+ //VANUI
  'Меняет различные текстуры и элементы интерфейса на тематические зимним праздникам.;'+ //HOLIDAY
  'Различные изменения баланса для настройки игры под себя. Не рекомендуются новичкам.;'+ //DIFFICULTY
  'Значительно снижает количество лома, дополнительно усложняя игру.;'+ //SCRAP STARVE
  'Сборник одобренных сторонних аддонов, не разрабатываемых командой Multiverse.;'+ //THIRD PARTY
  'Добавляет сгенерированные программно обломки, на которые будут рассыпаться корабли. Внимание: Увеличивает время запуска примерно на 20% и также увеличивает количество требуемой памяти.;'+ //GEN GIBS
  '[Обязательно] Основное API Hyperspace, позволяющее создание новых кораблей, экипажа, улучшенную манипуляцию событиями и т.д.;'+ //HS
  '[Обязательно] Дополнительное Lua API, позволяющее писать скрипты на Lua;' //LUA
  );
