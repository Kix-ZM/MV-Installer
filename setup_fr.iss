...
[Types]
Name: "compact"; Description: "Installation standard"
Name: "custom"; Description: "Installation Customizée"; Flags: iscustom

[Components] 
Name: "MultiverseCore"; Description: "Mod centraux"; Types: compact custom; Flags: fixed

Name: "MultiverseCore\TRC"; Description: "Collection Renégate"; Types: custom
Name: "MultiverseCore\SlockFlag"; Description: "'Le vaisseau mère de Slocknog"; Types: custom
Name: "MultiverseCore\VanillaUI"; Description: "Restaure l'interface standard"; Types: custom
Name: "MultiverseCore\Holiday"; Description: "Le spécial Fête de Noël"; Types: custom

Name: "MultiverseDiff"; Description: "Mod de Difficulté"; Types: custom
Name: "MultiverseDiff\Starvation"; Description: "Reduction de Scrap"; Types: custom

Name: "MultiverseThr"; Description: "Addons tiers"; Types: custom
Name: "MultiverseThr\GenGibs"; Description: "Géneration d'épave"; Types: custom

Name: "Hyperspace"; Description: "L'API de l'hyperespace"; Types: compact custom; Flags: fixed
Name: "Hyperspace\Lua"; Description: "API LUA"; Types: compact custom; Flags: fixed
...
[Run]
Filename: {app}\downgrade.bat; Description: Run Application; StatusMsg: "Mise à niveau vers FTL 1.6.9..."; Flags: runhidden
Filename: "{app}\modman.exe"; Parameters: {code:GetModmanParameters}; WorkingDir: "{app}"; StatusMsg: "Installation des mods sélectionnés..."; Flags: runhidden
Filename: "{app}\FTLGame.exe"; Description: "{cm:LaunchProgram,Multiverse}"; Flags: nowait postinstall skipifsilent runascurrentuser

[UninstallRun]
Filename: "{app}\modman.exe"; Parameters: "--patch ~none~"; StatusMsg: "Désinstallation de Multiverse..."; Flags: runhidden
...
begin
  ...
  if not IsJREInstalled then begin
    if MsgBox('La version 1.6 ou supérieure de Java est requise pour installer le mod. Aller à la page de téléchargement ?', mbConfirmation, MB_YESNO) = IDYES then begin
      ShellExec('open', 'https://www.java.com/en/download/', '', '', SW_SHOWNORMAL, ewNoWait, ErrorCode);
    end;
    while true do begin
      MBResult := MsgBox('Après avoir installé Java, cliquez sur "Réessayer" pour poursuivre l'installation du mod.', mbConfirmation, MB_ABORTRETRYIGNORE or MB_DEFBUTTON2);
      if MBResult = IDAbort then begin
        Result := false;
        MsgBox('L'installation a été interrompue par l'utilisateur.', mbConfirmation, MB_OK);
        exit;
      end;
      if (MBResult = IDIgnore) and (MsgBox('L'installation du mod ne fonctionnera pas sans Java. Êtes-vous sûr de vouloir ignorer l'installation et continuer malgré tout ?', mbError, MB_YESNO or MB_DEFBUTTON2) = idYes) then break;
...
begin
  ...
  if (CurPageID = wpSelectDir) and not TryPath(WizardDirValue) then begin
    Result := MsgBox('Impossible de trouver FTLGame.exe dans' + WizardDirValue + 'Pour une installation correcte de Multiverse, vous devez sélectionner le dossier d'installation du jeu. Voulez-vous quand même continuer ?',
...
begin
  ...
  '[Obligatoire] Contient des fichiers de données et des fichiers graphiques distincts pour le mod Multiverse..;'+ //CORE
  'Ajoute un nouvel arsenal d'armes, de butin potentiel et de vaisseaux jouables supplémentaires autour de ces armes, qui se concentrent sur des gimmick plus uniques avec moins d'attention à l'équilibre..;'+ //TRC
  'Ajoute un type B très puissant et déverrouillable au joueur, le "Pleasure Flagship". Créé à l'origine comme prix pour le concours de Noël 2021 sur le Discord..;'+ //SLOCKNOG'S FLAGSHIP
  'Remplace l'interface utilisateur personnalisée de Multiverse et rétablit l'interface FTL standard. Nous vous recommandons d'essayer d'abord l'interface Multiverse par défaut..;'+ //VANUI
  'Remplacement de diverses textures et éléments de l'interface utilisateur par des éléments sur le thème des vacances d'hiver..;'+ //HOLIDAY
  'Diverses modifications mineures de l'équilibre pour personnaliser davantage votre expérience. Non recommandé pour les nouveaux joueurs.;'+ //DIFFICULTY
  'Réduit considérablement les gains de scrap pour ajouter un défi supplémentaire au jeu..;'+ //SCRAP STARVE
  'Une collection d'addons tiers approuvés, non développés par l'équipe centrale de Multiverse..;'+ //THIRD PARTY
  'Ajoute des épaves générés de manière programmatique, empêchant de nombreux types de vaisseaux de simplement disparaître lorsqu'ils sont détruits. Note : Augmente le temps de démarrage d'environ 20% et augmente également la consommation de mémoire..;'+ //GEN GIBS
  '[Obligatoire] L'API de base de l'hyperespace qui permet la création de nouveaux vaisseaux de joueurs, d'équipages, la manipulation avancée d'événements, etc.;'+ //HS
  '[Obligatoire] API Lua supplémentaire qui permet l'utilisation de scripts Lua.;' //LUA
  );
